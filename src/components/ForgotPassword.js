...
export default function ForgotPassword({ move, setShowForgetPassword, showForgetPassword }) {
  ...
  const passwordVerifyHandler = async () => {
    ...
    if (forgotPasswordFormik.values.email) {
      await dispatch({ type: "SET_OTP", payload: body, otpPayload: "forgot_password" });
    } else {
